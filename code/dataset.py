...
transform_options = {
    ...
    "CIFAR100": {
         ...
         "test_transform": [transforms.ToTensor()]},
    "SVHN": {
        "train_transform": [transforms.ToTensor()],#SVHN只有最普通的transforms，可能也是一个原因
        "test_transform": [transforms.ToTensor()]},
    "ImageNet": {
...
class DatasetGenerator():
    def __init__(self, train_batch_size=128, eval_batch_size=256, num_of_workers=4,
                 ...
                 no_train_augments=False, poison_rate=1.0, mask_rate = 1.,perturb_type='classwise',
        ...
        if train_data_type == 'CIFAR10':
            ...
            train_dataset = PoisonCIFAR10(root=train_data_path, transform=train_transform,
                                          poison_rate=poison_rate, mask_rate=mask_rate,perturb_type=perturb_type,
            ...
            train_dataset = PoisonImageNetMini(root=train_data_path, split='train', seed=seed,
                                               transform=train_transform, poison_rate=poison_rate,mask_rate=mask_rate,
                                               perturb_tensor_filepath=perturb_tensor_filepath)
        ...
        elif train_data_type =='WebFace-100':
            train_dataset = datasets.ImageFolder(root=train_data_path+'/webface-100-train', transform=train_transform)
        elif train_data_type =='WebFace-10':
            train_dataset = datasets.ImageFolder(root=train_data_path+'/webface-10-train', transform=train_transform)
        elif train_data_type =='PoisonWebFace-10':
            train_dataset = PoisonWebFace10(root=train_data_path+'/webface-10-train', transform=train_transform,pertub_tensor_filepath=perturb_tensor_filepath)
            #train_dataset = PoisonWebFace10(root)
        ...
        if test_data_type == 'CIFAR10':
            ...
            test_dataset = PoisonImageNetMini(root=test_data_path, split='val', seed=0,
                                              transform=test_transform, poison_rate=poison_rate,mask_rate=mask_rate,
                                              perturb_tensor_filepath=perturb_tensor_filepath)
            ...
            test_dataset = CatDogDataset(root=train_data_path, split='train',
                                         transform=train_transform)
        elif test_data_type == 'WebFace-100':
            test_dataset = datasets.ImageFolder(root=test_data_path+'/webface-100-test', transform=test_transform)
        elif test_data_type == 'WebFace-10':
            test_dataset = datasets.ImageFolder(root=test_data_path+'/webface-10-test', transform=test_transform)

        elif test_data_type == 'CelebAMini' or 'CelebA':
            pass
        elif test_data_type == 'FaceScrub':
            pass
        else:
...
class PoisonCIFAR10(datasets.CIFAR10):
    def __init__(self, root, train=True, transform=None, target_transform=None,
                 download=False, poison_rate=1.0,mask_rate=1., perturb_tensor_filepath=None,
        ...
        # low = 1.-1./mask_rate
        # noise_mask = np.random.uniform(low=low,high=1.,size = (32,32,1))
        # noise_mask = ((np.repeat(noise_mask,repeats=3,axis=2))>0).astype(np.float)#得到了mask，在3的维度上一致
        # print(noise_mask)
        # print(noise_mask.sum()/3/1024)
...
class PoisonWebFace10(datasets.ImageFolder):
    
    def __init__(self, root,transform,pertub_tensor_filepath=None):
        super(PoisonWebFace10,self).__init__(root, transform=transform)
        self.perturb_tensor = torch.load(pertub_tensor_filepath,map_location=device).to('cpu')


    def __getitem__(self, index: int):
        ori_img,label = super().__getitem__(index)
        
        img = self.perturb_tensor[index]+ori_img
        

        clap_img = img.clamp_(min=0,max=1.0)
        
        return clap_img,label
...
class PoisonCIFAR100(datasets.CIFAR100):
    def __init__(self, root, train=True, transform=None, target_transform=None,
        ...
        for idx in self.poison_samples_idx:
            ...
            self.data[idx] += noise#*noise_mask
...
class PoisonImageNetMini(ImageNetMini):
    def __init__(self, root, split, poison_rate=1.0, mask_rate=1.,seed=0,
...
def generate_web_face():
    import os
    from shutil import copy
    webface_dirs = os.listdir('../datasets/webface/CASIA-WebFace')
    if not os.path.exists('../datasets/webface-100'):
        os.mkdir('../datasets/webface-100')
    #webface_dirs = sorted(webface_dirs)
    for i,dir in enumerate(webface_dirs):
        if i>=100:return
        tgt_dir = '../datasets/webface-100/'+dir
        
        src_dir = '../datasets/webface/CASIA-WebFace/'+dir
        if not os.path.exists(tgt_dir):
            os.mkdir(tgt_dir)
        filenames = os.listdir(src_dir)
        for f in filenames:
            copy(src_dir+'/'+f,tgt_dir)
            
        
def webface_split(num_classes=10):
    import os
    from sklearn.model_selection import train_test_split
    from shutil import copy
    if not os.path.exists('../datasets/webface-%s-train'%num_classes):
        os.mkdir('../datasets/webface-%s-train'%num_classes)
    if not os.path.exists('../datasets/webface-%s-test'%num_classes):
        os.mkdir('../datasets/webface-%s-test'%num_classes)

    dirs = os.listdir('../datasets/webface-%s'%num_classes)
    for dir in dirs:
        if not os.path.exists('../datasets/webface-%s-train'%num_classes +'/'+dir):
            os.mkdir('../datasets/webface-%s-train'%num_classes+'/'+dir)
        if not os.path.exists('../datasets/webface-%s-test'%num_classes+'/'+dir):
            os.mkdir('../datasets/webface-%s-test'%num_classes+'/'+dir)

        files = os.listdir('../datasets/webface-%s'%num_classes+'/'+dir)
        train_files, test_files = train_test_split(files,test_size=0.2,random_state=0)
        for file in train_files:
            copy('../datasets/webface-%s'%num_classes+'/'+dir+'/'+file,'../datasets/webface-%s-train'%num_classes+'/'+dir)
        for file in test_files:
            copy('../datasets/webface-%s'%num_classes+'/'+dir+'/'+file,'../datasets/webface-%s-test'%num_classes+'/'+dir)
    



def generate_web_face_10():
    import os
    from shutil import copy
    webface_dirs = os.listdir('../datasets/webface/CASIA-WebFace')
    if not os.path.exists('../datasets/webface-10'):
        os.mkdir('../datasets/webface-10')
    dir2num={}
    for dir in webface_dirs:
        img_num = len(os.listdir('../datasets/webface/CASIA-WebFace/'+dir))
        dir2num[dir]=img_num
    dir_num = sorted(dir2num.items(),key = lambda x:x[1],reverse = True)
    print(dir_num[:10])
    
    
    n=0
    for i,(dir,num) in enumerate(dir_num):
        n+=num
        if i>=10:break
        tgt_dir = '../datasets/webface-10/'+dir
...
def count_num():
    import os
    dirs = os.listdir('../datasets/webface-10-train')
    n=0
    for dir in dirs:
        files = os.listdir('../datasets/webface-10-train/'+dir)
...
if __name__ == '__main__':
    train_dataset = PoisonWebFace10(root='../datasets/webface-10-train', transform=transforms.Compose([
                            transforms.Resize(size=(224,224)),
                            transforms.ToTensor()]),pertub_tensor_filepath='../experiments/mada/samplewise/webface-10/at_pretrain_epsilon32/perturbation/resnet18_madrys/perturbation.pt')
    print(len(train_dataset))

    test_dataset = datasets.ImageFolder(root='../datasets/webface-10-test',transform=transforms.Compose([
                            transforms.Resize(size=(224,224)),
                            transforms.ToTensor()]))

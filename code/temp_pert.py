...
parser.add_argument('--attack_type', default='min-min', type=str, choices=['min-min', 'min-max', 'random', 'mada' ], help='Attack type')
...
parser.add_argument('--step_size', default=0.8, type=float, help='perturb step size')#乘微扰值的超参数
parser.add_argument('--random_start', action='store_true', default=False)
parser.add_argument('--transfer_train',action='store_true',default=False)#用于在train from scratch的时候选择是否transer label（默认不选择）

parser.add_argument('--noise_shape', default=[10, 3, 32, 32], nargs='+', type=int, help='noise shape')#可以控制局部被poisoned
parser.add_argument('--noise_portion',type=int,default=1)#1/noise_protion是作为产生noise的分母
...
if torch.cuda.is_available():
    ...
    device = torch.device('cuda:5')#用于保存数据的特定gpu
    gpus = [5]
...
inverse_label_fucntion = torch.arange(0, config.num_classes, dtype=torch.long, device=device)

inverse_label_fucntion = (inverse_label_fucntion + config.num_classes - 1)%config.num_classes


target_label_function = torch.LongTensor([8,9,7,5,6,3,4,2,0,1]).to(device)

inverse_label_fucntion = torch.LongTensor([8,9,7,5,6,3,4,2,0,1]).to(device)
    ...
    while condition:
        if (args.attack_type == 'min-min' or args.attack_type == 'mada') and not args.pretrain:
            ...
            for j in range(0, args.train_step):
                ...
                if args.attack_type == 'mada' and args.transfer_train == True :#如果是mada且transfer的情形下，from scratch train的时候用的是(x,y+1)
...
def sample_wise_perturbation(noise_generator, trainer, evaluator, model, criterion, optimizer, scheduler, random_noise, ENV):
    ...
    while condition:
        ...
        # Search For Noise
        idx = 0
        for i, (images, labels) in tqdm(enumerate(data_loader['train_dataset']), total=int(len(data_loader['train_dataset'])/args.noise_portion) ):
            images, labels, model = images.to(device), labels.to(device), model.to(device)
...
def main():
    ...
    elif args.attack_type == 'min-min' or args.attack_type == 'min-max'  or args.attack_type == 'mada':

    ...
    while condition:
        ...
        for i, (images, labels) in tqdm(enumerate(data_loader['train_dataset']), total=len(data_loader['train_dataset'])):
            ...
            if args.attack_type == 'min-min':
                ...
                plus_one_labels = torch.arange(1, 11, dtype=torch.long, device=labels.device)
                plus_one_labels[-1] = 0
                attack_labels = plus_one_labels[labels]
...
def main():
    ...
    if args.attack_type == 'random':
        ...
        if args.attack_type == 'min-max' or args.attack_type == 'mada':

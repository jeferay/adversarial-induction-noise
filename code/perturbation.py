...
from label_assign import least_similar_swap, random_swap, similar_swap
...
parser.add_argument('--version', type=str, default="resnet18")#网络以及训练方式的version
parser.add_argument('--exp_name', type=str, default="test_exp")
parser.add_argument('--config_path', type=str, default='configs/cifar10')
parser.add_argument('--load_model', action='store_true', default=False)#这里的load model指的是用已经train好的model找noise
...
parser.add_argument('--pretrain_epochs', default=100, type=int, help='pretrain epochs')#预训练的epoch数
...
parser.add_argument('--attack_type', default='min-min', type=str, choices=['min-min', 'min-max', 'random', 'mada','sample_assign' ], help='Attack type')
...
parser.add_argument('--step_size', default=0.8, type=float, help='perturb step size')#乘微扰值的超参数（0.8/255）
parser.add_argument('--random_start', action='store_true', default=False)

parser.add_argument('--label_assign',type=str, default='simiar_swap',choices=['similar_swap','near_swap','random_swap','plus_one','least_similar_swap','MC','LL','random'])


parser.add_argument('--noise_shape', default=[10, 3, 32, 32], nargs='+', type=int, help='noise shape')#可以控制局部被poisoned


parser.add_argument('--label_noise',action='store_true',default=False)#用于决定是否产生label noise，如果产生的话将新的所有label存入一个文件
parser.add_argument('--iteration_step',type=int,default=0) #控制最小的迭代次数，五次
...
# create plus_one_labels and minus_one_labels for mada attack
target_label_function = torch.arange(1, config.num_classes+1, dtype=torch.long, device=device)
target_label_function[-1] = 0



if args.label_assign=='near_swap':
    target_label_function = torch.LongTensor([i+1 if i%2==0 else i-1 for i in range(config.num_classes)]).to(device)
    
elif args.label_assign=='similar_swap':
    target_label_function = similar_swap(matrix=None,dataset_name= args.test_data_type).to(device)
elif args.label_assign =='least_similar_swap':
    target_label_function = least_similar_swap(matrix=None,dataset_name= args.test_data_type).to(device)
elif args.label_assign == 'random_swap':
    target_label_function = random_swap(args.test_data_type).to(device)

if args.label_assign in ['random','MC','LL']:
    target_labels = torch.load('../experiments/confusion_martix/'+args.label_assign+'.pth').to(device)


logger.info(target_label_function)
...
def train(starting_epoch, model, optimizer, scheduler, criterion, trainer, evaluator, ENV, data_loader):
    for epoch in range(starting_epoch, args.pretrain_epochs):
        logger.info("")
...
def universal_perturbation_eval(noise_generator, random_noise, data_loader, model, eval_target=args.universal_train_target):
    ...
    for i, (images, labels) in enumerate(data_loader[eval_target]):
        if args.attack_type == 'mada':
            labels = target_label_function[labels]#对于mada做target label accu评测
...
def universal_perturbation(noise_generator, trainer, evaluator, model, criterion, optimizer, scheduler, random_noise, ENV):
    ...
    if hasattr(model, 'classify'):
        model.classify = True
    while condition:
        if (args.attack_type == 'min-min' or args.attack_type == 'mada') and not args.pretrain:
            # Train Batch for noise
            for j in tqdm(range(0, args.train_step), total=args.train_step):
                ...
                if args.attack_type == 'mada':#如果是mada且transfer的情形下，from scratch train的时候用的是(x,y+1)
                    labels = target_label_function[labels]
        ...
        for i, (images, labels) in tqdm(enumerate(data_loader[args.universal_train_target]), total=len(data_loader[args.universal_train_target])):
            ...
            elif args.attack_type == 'mada':
                attack_labels = target_label_function[labels]
                perturb_img, eta = noise_generator.min_min_attack(images, attack_labels, model, optimizer, criterion, random_noise=batch_noise)
        ...
        if args.attack_type == 'min-min' or args.attack_type == 'mada':
            condition = error_rate > args.universal_stop_error
        elif args.attack_type == 'min-max':#因为已经预训练过，而产生noise的目标是使得模型失效，所以要增大error-rate
            condition = error_rate < args.universal_stop_error
        if args.pretrain ==True:
            condition = False#对于pretrain直接停机
    return random_noise

#该函数没有用到evaluator的模块，因为只在perturbation一次iteration之后执行，因此单次执行eval
def samplewise_perturbation_eval(random_noise, data_loader, model, eval_target='train_dataset', mask_cord_list=[]):
    ...
    for i, (images, labels) in enumerate(data_loader[eval_target]):
        ...
        if args.attack_type == 'sample_assign':#对于assign_sample也做相应的评测
            labels=target_labels[idx:idx+batch_size]
        if args.attack_type == 'mada':
            labels = target_label_function[labels]#对于mada做target label accu评测
        images, labels = images.to(device, non_blocking=True), labels.to(device, non_blocking=True)
        if random_noise is not None:
...
def sample_wise_perturbation(noise_generator, trainer, evaluator, model, criterion, optimizer, scheduler, random_noise, ENV):
    ...
    iteration_step=0
    while condition or iteration_step<args.iteration_step:
        iteration_step+=1
        if (args.attack_type == 'min-min' or args.attack_type == 'mada') and not args.pretrain:#运行M的训练来修正model
            # Train Batch for min-min noise，此时是非std训练
            for j in tqdm(range(0, args.train_step), total=args.train_step):
                try:
                    (images, labels) = next(data_iter)
                except:
                ...
                if args.attack_type == 'mada':#如果是mada且transfer的情形下，from scratch train的时候用的是(x,y+1)
                    labels = target_label_function[labels]
                images, labels = images.to(device), labels.to(device)
                # Add Sample-wise Noise to each sample #非std训练
                for i, (image, label) in enumerate(zip(images, labels)):
        ...
        # Search For Noise，按照batch寻找
        idx = 0#这个idx是针对全局数据的idx
        for i, (images, labels) in tqdm(enumerate(data_loader['train_dataset']), total=len(data_loader['train_dataset'])):
            ...
            if args.attack_type == 'min-min':
                ...
                attack_labels = target_label_function[labels]
                perturb_img, eta = noise_generator.min_min_attack(images, attack_labels, model, optimizer, criterion, random_noise=batch_noise)
            elif args.attack_type == 'sample_assign':
                batch_size = images.shape[0]
                attack_labels = target_labels[batch_start_idx:batch_start_idx+batch_size]
                perturb_img, eta = noise_generator.min_min_attack(images, attack_labels, model, optimizer, criterion, random_noise=batch_noise)
        ...
        # Eval termination conditions
        loss_avg, error_rate = samplewise_perturbation_eval(random_noise, data_loader, model, eval_target='train_dataset',#这里的target是train dataset
        ...
        if torch.is_tensor(random_noise):
            random_noise = random_noise.detach()
            ENV['random_noise'] = random_noise
        if args.attack_type  in ['min-min','mada','sample_assign']:
            condition = error_rate > args.universal_stop_error
            if args.pretrain:condition = False#对于pretrian，只运行一轮200step
        elif args.attack_type == 'min-max':
            condition = error_rate < args.universal_stop_error
...
def main():
    ...
    criterion = config.criterion()#这里可以确定迭代过程中和pretrain是否用at
    if args.perturb_type == 'samplewise':
        train_target = 'train_dataset'
    else:
        if args.use_subset:#只有在use_subset的时候才起作用
            data_loader = datasets_generator._split_validation_set(train_portion=args.universal_train_portion,
                                                                   train_shuffle=True, train_drop_last=True)
    ...
    if args.attack_type == 'random':
        ...
        torch.save(noise, os.path.join(exp_path, 'perturbation.pt'))
        logger.info(noise)
        logger.info(noise.shape)
        logger.info('Noise saved at %s' % (os.path.join(exp_path, 'perturbation.pt')))
    elif args.attack_type in ['min-min','min-max','mada','sample_assign']:
        #mada区分pretrain和from scratch
        if args.pretrain and not args.load_model:#load model仅可能发生在pretrained的时候
            # min-max noise need model to converge first
            
            train(0, model, optimizer, scheduler, criterion, trainer, evaluator, ENV, data_loader)#根据criterion选择madry loss
        if args.pretrain:
            evaluator.eval(epoch = 100, model = model,target_labels= None)#常规的eval
        ...
        criterion = torch.nn.CrossEntropyLoss()#生成噪声的阶段要重新选用criterion（不能是madry_loss，因为madry_loss不对应于本身点的特征）

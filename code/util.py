...
#返回accu和accuk两个值，accuk指的是前k个包含正确选项则正确
def accuracy(output, target, topk=(1,)):
    maxk = max(topk)

    batch_size = target.size(0)
    _, pred = output.topk(maxk, 1, True, True)#返回在dim1上最大的maxk个值并且倒序排序，返回排序好的值以及对应原来的的index
    pred = pred.t()#转置
    correct = pred.eq(target.view(1, -1).expand_as(pred))#求eq比较是否是argmax或者是否是在被topk包含

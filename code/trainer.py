...
class Trainer():
    ...
    #这里是standard train，一个epoch的过程，并不是整个training，整个train函数在main.py或者perturbation.py
    def train(self, epoch, model, criterion, optimizer, random_noise=None):
        """
        epoch：当前所处的epoch，用于log
        random_noise:这里是指poison，已经提前计算好
        """
        model.train()
        #按照batch
        for j, (images, labels) in enumerate(self.data_loader[self.target]):
    ...
    def train_batch(self, images, labels, model, optimizer):
        ...
        else:# madrysloss/martloss/tradesloss
